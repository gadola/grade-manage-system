...
const sendEmail = async ({ to, subject, text, html, ...rest }) => {
  try {
    ...
    if (res) {
      ...
      const mail = {
        //sender access
        from: '"Grade management system" <no-reply@accounts.gms.com>',
        //receiver access
        to,
        //subject
        subject,
...
const htmlSignupAccount = (token) => {
  return `<div>
    ...
    <h2 style="padding: 10px 0; margin-bottom: 10px;">
        Xin chào anh (chị),<br />
        Mã xác nhận đăng ký tài khoản cho website GMS của anh (chị).<br />
        Cảm ơn./.
    </h2>
...
const htmlResetPassword = (token) => {
  return `<div>
    ...
    <h2 style="padding: 10px 0; margin-bottom: 10px;">
        Xin chào anh (chị),<br />
        GMS đã nhận được yêu cầu lấy lại mật khẩu từ quý khách.<br />
